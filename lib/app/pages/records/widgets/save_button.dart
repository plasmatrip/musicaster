...
class SaveButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onTap: context.watch<RecordsRepo>().canSave()
          ? () async {
              context.read<RecordsRepo>().save();
              AutoRouter.of(context).maybePop(true);
            }
          : null,
      child: Ink(
        decoration: ShapeDecoration(
          color: context.read<RecordsRepo>().canSave() ? null : const Color(0xFFD3D3D3),
          gradient: context.read<RecordsRepo>().canSave()
        ...
        child: SizedBox(
          ...
          child: Center(
            child: Text(
              'Save',
              style: context.s17w600.copyWith(color: context.read<RecordsRepo>().canSave() ? white : greyDark),
            ),
          ),
        ),
      ),
